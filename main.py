...
from telegram.ext import ContextTypes, MessageHandler, filters, CallbackQueryHandler, Updater, CallbackContext
...
def echo(update: Update, context: CallbackContext) -> None:
    ...
    if(message_text == "/start"):
        ...
        message.reply_text("Вас приветсвует бот от компании лукойл 😀",reply_markup=keyboard_0)
        ...
        message.reply_photo("https://avatars.mds.yandex.net/i?id=04f34f9bb4609414ca491a3130d6b2b9aeedc22e-9083123-images-thumbs&n=13", "1.   Чем занимается компания “Лукойл”?", reply_markup=keyboard1)
        ...
        if message_text == "Добычей нефти и газа":
            time.sleep(0.5)
            message.reply_text("Верно👌")
            time.sleep(0.5)
            count[username]+=1
        else:
            time.sleep(0.5)
            message.reply_text("Неверно😢")
            time.sleep(0.5)
        # new question
        ...
        message.reply_photo("https://static.tildacdn.com/tild3230-3466-4163-a532-326536373531/---.jpg", "2.	В скольких странах мира Лукойл занимается добычей и разведкой?", reply_markup=keyboard3)
        # countable
        countable[username] += 1
        ...
        message.reply_text("Вы прошли тест!", reply_markup=keyboardAg)
        message.reply_photo(
            "https://fikiwiki.com/uploads/posts/2022-02/1644725019_24-fikiwiki-com-p-yunost-krasivie-kartinki-28.jpg",
            "")
        message.reply_text("Идёт подсчёт правильных ответов🤔")
        time.sleep(2)
        if(count[username]>=8):
            message.reply_text("Поздравляю у вас "+str(count[username])+" баллов")
            message.reply_text("За такой хороший результат вам положена награда)❤")
            time.sleep(2)
            message.reply_text("http://t.me/addstickers/lukoil_stickers_by_fStikBot")
            message.reply_text("Это ссылка на наш стикер пак😍")
        else:
            message.reply_text("Поздравляю у вас " + str(count[username]) + " баллов")
            message.reply_text("У вас не получилось набрать нужное количество баллов(😭")
            time.sleep(1)
            message.reply_text("Попробуйте снова")
        countable[username] = -1
        count[username] = 0
        ...
        if(message_text == "Вперёд"):
            ...
            message.reply_text("Вы вышли из презентации", reply_markup=keyboard_0)
        else:
            message.reply_text("Такого варианта не существует😯")
    ...
    message.reply_photo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRpeGrn047P2tXQo4lvL8x0WC0TKph2-mWhxK9gziE&s","3.	Сколько нефтедобывающих скважин в арсенале Лукойла?", reply_markup=keyboard4)
    ...
    message.reply_photo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRpeGrn047P2tXQo4lvL8x0WC0TKph2-mWhxK9gziE&s", "4.	От первых букв названия каких городов происходит ЛУК в названии Лукойл? ", reply_markup=keyboard5)
    ...
    #message.reply_photo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRpeGrn047P2tXQo4lvL8x0WC0TKph2-mWhxK9gziE&s", "Дароу старый!")
    #message.reply_photo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRFLYH79n5BowDE27E0gnHoJA1PA29dqCeBlDQYaFo&s","Не понимать(")


def main() -> None:
    updater = Updater("<PASTE_YOUR_TELEGRAM_TOKEN_HERE>")
